...
use crate::cloth::{Cloth, ClothCollision, ClothTearing, ClothWind};
...
#[allow(clippy::needless_pass_by_value)]
pub fn cloth_constraints_system(mut cloth_query: Query<(&mut Cloth, Option<&ClothTearing>)>) {
        ...
        for _ in 0..ITERATIONS {
            ...
            for (idx, constraint) in constraints_snapshot.iter().enumerate() {
                ...
                if let Some(tearing_settings) = tearing {
                    if tearing_settings.enabled
                        && distance > constraint.rest_length * tearing_settings.tear_distance
                    {
                ...
                if distance > 0.0001 && (inv_mass_a > 0.0 || inv_mass_b > 0.0) {
                    ...
                    if inv_mass_a > 0.0 {
                        cloth.particles[particle_a].position +=
                            correction * (inv_mass_a / total_inv_mass);
                    }

                    if inv_mass_b > 0.0 {
                        cloth.particles[particle_b].position -=

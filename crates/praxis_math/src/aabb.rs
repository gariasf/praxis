...
impl Aabb {
    ...
    ///
    /// Ensures min <= max by swapping components if necessary.
    ...
    /// assert_eq!(aabb.max, Vec3::splat(6.0));
    /// ```
    pub fn from_center_half_extents(center: Vec3, half_extents: Vec3) -> Self {
        ...
        let half_extents = if half_extents.is_finite() {
            half_extents.abs()
        } else {
            Vec3::ONE
        };
    ...
    ///
    /// An empty AABB has inverted min/max values, which makes any union
    /// operation replace it with valid bounds.
    pub fn empty() -> Self {
        Self {
    ...
    /// Tests if a point is inside the AABB (inclusive).
    ...
    ///
    /// Computes the new AABB that bounds all 8 transformed corners.
    ...
    ///
    /// If the point is inside the AABB, returns 0.0.
...
mod tests {
    ...
    fn test_aabb_distance() {
        ...
        assert!((aabb.distance(point) - 1.0).abs() < EPSILON);
    }

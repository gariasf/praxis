...
mod tests {
    ...
    fn test_exposure_calculator_clamping_min() {
        ...
        assert!(
            final_exposure >= 0.1,
            "Exposure should not go below min (0.1)"
        );
        assert!(
            (final_exposure - 0.1).abs() < 0.01,
            "Should clamp to min exposure"
        );
    }
    ...
    fn test_exposure_calculator_clamping_max() {
        ...
        assert!(
            final_exposure <= 10.0,
            "Exposure should not exceed max (10.0)"
        );
        assert!(
            (final_exposure - 10.0).abs() < 0.1,
            "Should clamp to max exposure"
        );
    }
    ...
    fn test_exposure_calculator_very_small_luminance() {
        ...
        assert_eq!(
            final_exposure, 1.0,
            "Very small luminance should result in default exposure"
        );
    }

...
// Lighting Constants
// ============================================================================
// These could be moved to the uniform buffer in the future for per-material control

// Specular shininess/power (higher = smaller, sharper highlights)
// Typical ranges: 2-4 (dull plastic), 32-64 (polished), 128+ (mirror-like)
...
void main() {
    ...
    // Vertex colors can tint or modulate the texture
    ...
    for (uint i = 0; i < lighting.directional_light_count; i++) {
        ...
        // (specular is typically less intense than diffuse)
    ...
    // ========================================================================
    // Step 6: Apply Lighting to Albedo
    // ========================================================================
    
    // Multiply the accumulated lighting by the surface's base color
    // This gives the final lit color: lighting * albedo
    ...
    // Output final color with alpha from the texture
    // Alpha channel passes through unchanged (for transparency support)

...
pub fn register_engine_api(lua: &Lua) -> Result<()> {
    ...
    engine_table.set("log_info", lua.create_function(|_, msg: String| {
        praxis_utils::info!("[Script] {}", msg);
        Ok(())
    })?)?;
    
    engine_table.set("log_debug", lua.create_function(|_, msg: String| {
        praxis_utils::debug!("[Script] {}", msg);
        Ok(())
    })?)?;
    
    engine_table.set("log_warn", lua.create_function(|_, msg: String| {
        praxis_utils::warn!("[Script] {}", msg);
        Ok(())
    })?)?;
    
    engine_table.set("log_error", lua.create_function(|_, msg: String| {

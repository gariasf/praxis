...
use praxis_scripting::{
    ScriptingConfig, ScriptingContext,
...
fn demo_math_api(context: &mut ScriptingContext) -> praxis_utils::Result<()> {
    ...
    context.load_string("math_test", r#"
    ...
    let (x, y, z): (f32, f32, f32) = context.call_function(
        "math_test",
        "create_vector",
        ()
    )?;
...
fn demo_performance_monitoring(context: &ScriptingContext) -> praxis_utils::Result<()> {
    ...
    context.load_string("perf_test", r#"
    ...
    if let Some(monitor) = context.performance_monitor() {
        ...
        println!("{:<40} {:<15} {:<15} {:<15}", "Function", "Avg Time", "Min Time", "Max Time");
        println!("{}", "-".repeat(85));
        
        for stats in monitor.get_all_stats() {
            let func_name = format!("{}::{}", stats.script_name, stats.function_name.unwrap_or_default());
            println!(
                "{:<40} {:<15.3?} {:<15.3?} {:<15.3?}",
                func_name,
                stats.average_time,

...
use praxis_ecs::{World, Transform, Schedule};
use praxis_math::{Vec3, Quat};
use praxis_networking::{
    NetworkServer, NetworkClient, NetworkConfig, ReplicationRegistry,
    NetworkId, Replicated, ReplicatedTransform, ReplicatedVelocity,
    NetworkInterpolation, InterpolationBuffer, InterpolationSystem,
    NetworkExtrapolation, ExtrapolationSystem, NetworkProfiler,
    LagCompensation, LagCompensationSystem,
};
...
async fn run_server() -> color_eyre::Result<()> {
    ...
    for _ in 0..600 {  // Run for 10 seconds
        let now = Instant::now();
        ...
        if delta >= tick_duration.as_secs_f32() {
            ...
            if tick_count % 60 == 0 {
                println!("Tick {}: {} clients connected", 
                    tick_count, 
                    server.client_count()
                );
            }
...
async fn run_client() -> color_eyre::Result<()> {
    ...
    println!("  Interpolation delay: {} ms", config.interpolation_delay_ms);
    ...
    client.disconnect("Client shutting down".to_string()).await?;
...
fn demonstrate_profiler() {
    ...
    println!("  Receive rate: {:.2} bytes/sec", stats.bandwidth.receive_rate);
    println!("  Peak send rate: {:.2} bytes/sec", stats.bandwidth.peak_send_rate);
    println!("  Peak receive rate: {:.2} bytes/sec", stats.bandwidth.peak_receive_rate);
    println!();
    println!("Latency:");
    ...
    match lag_comp.rewind_to_client_time(1, 300, &mut world) {
        Ok(rewind_state) => {
            ...
            if let Some(transform) = world.get::<ReplicatedTransform>(entity) {
                println!("Entity position after rewind: ({}, {}, {})",
